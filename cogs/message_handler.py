...
class MessageHandler(utils.Cog):
    ...
    async def on_message_edit(self, before:discord.Message, after:discord.Message):
        ...
        with utils.Embed(colour=0x0000ff) as embed:
            ...
            if len(before.content) > 750:
                before_content = before.content[:750] + '...'
            after_content = after.content
            if len(after.content) > 750:
                after_content = after.content[:750] + '...'
    ...
    async def on_message_delete(self, message:discord.Message):
        ...
        with utils.Embed(use_random_colour=True) as embed:
            ...
            embed.set_footer(f"User ID: {message.author.id} | Message ID: {message.id}")
        ...
        if message.guild.me.guild_permissions.view_audit_log:
            ...
            async for entry in message.guild.audit_logs(action=discord.AuditLogAction.message_delete, limit=1):
                ...
                changed = True
                embed.description = f"Message by {message.author.mention} deleted in {message.channel.mention}"
